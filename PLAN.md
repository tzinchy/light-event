...
**Дополнение §11.4 (реализовано):** комиссия 6% удерживается из суммы выплаты (соискатель получает 94%); резерв available→on_hold внутри счёта в журнал не пишется — двойная запись только между счетами, on_hold сверяется по открытым payout; подтверждение соискателя без средств на резерв → 409 с откатом.

### 11.5 Разворот логина: только e-mail + OTP (решение заказчика 2026-07-04)

Заказчик: «пока отложи авторизацию по номеру телефона — пускай будет только почта + OTP». Это **отменяет §10.2** в части «вход только по телефону».

- **Логин-идентификатор — e-mail.** Канал OTP при входе — `email` (уже поддержан `OtpStore` и `EmailProvider`; в dev код уходит в лог/`Mailpit`). Телефон — необязательное поле профиля на будущее (не участвует во входе).
- **Модель `User`:** `phone` → nullable (уникальный, но необязательный); `email` → уникальный + индекс, проставляется как логин, `email_verified_at` = момент первого успешного verify. Миграция — только `alembic revision --autogenerate`.
- **`AuthService`:** `request_otp(email)` шлёт код через `email_provider`; `verify_otp(email, code)` → `get_by_email`/`create(email)` + `email_verified_at`. Схемы `/api/v1/auth/otp/{request,verify}` принимают `email: EmailStr` вместо `phone`. `MeOut/UserOut.phone` → optional.
- **Тесты:** `tests/auth/test_otp_auth.py` переписан на e-mail; фикстура `login_user` логинит по e-mail через `email_outbox` (телефоноподобные уникальные ключи в старых вызовах детерминированно разворачиваются в e-mail — чтобы не трогать ~40 мест). Остальные 130+ тестов остаются зелёными.
- **Web:** экран `/auth` — шаг «почта» вместо «телефон»; SDK regen.
- **Кабинеты (пояснение по вопросу заказчика):** кабинет `manager/main-manager` — это `/org` (сайдбар: Аналитика/События/Отклики/Тесты/Команда/Баланс/Отзывы). Вход: хедер «Открыть кабинет организации» → `/org`; нет компании → форма заявки на компанию (создатель = `main_manager`) либо инвайт `/join/<code>`. `/admin` — отдельная платформенная админка (`platform_role=admin`), не путать. Замечено: у `/admin` нет ни пункта навигации, ни фронт-гейта по роли — вынести в полировку §9 п.15.

### 11.6 Дизайн под референс + новые фичи (заказчик 2026-07-05)

**A. Кабинеты по макету референса (`Платформа найма - standalone.html`).** Токены (zinc+green, Geist) уже совпадают; расходилась композиция. Референс снят headless-браузером (сайдбар-шелл: серый фон → скруглённая панель → сайдбар с шапкой сверху и карточкой пользователя снизу → main с заголовком, 4 метрики, карточки).
- `/org` — **сделано**: shell + экран «Аналитика» (метрики/график/рейтинг) с честными пустыми состояниями (real-data-only; в референсе цифры фейковые — не переносим).
- `/admin` — **в работе**: тот же shell (бренд «Admin Console», сайдбар-разделы Обзор/Организации/Публикации/Пополнения/Выплаты/Жалобы, карточка админа снизу), лендинг «Обзор» = 4 реальные метрики (overview-эндпоинт есть) + колонки «Очередь верификаций» и «Жалобы». Кнопки разделов оставить `button` с прежними именами — от них зависит `e2e/org.spec.ts`.
- **Долг:** `e2e/helpers.ts` всё ещё логинит по телефону — сломано разворотом §11.5, переписать на e-mail+Mailpit (`loginByEmail`), обновить `grantAdmin` на e-mail.

**B. Редактирование тарифов админом.** Сейчас цены — константы в `Settings` (`vacancy_publish_fee_kop=99000`, `company_test_fee_kop=150000`, `platform_commission_pct=6`).
- Таблица `service_price(key uniq, amount_kop)` (ключи `vacancy_publish`, `company_test`; комиссия — отдельно или тем же механизмом). Без seed-строк: репозиторий читает БД, при отсутствии строки — фолбэк на дефолт из `Settings` (real-data-only).
- Эндпоинты `GET /admin/pricing`, `PUT /admin/pricing/{key}` (RBAC admin). Сервисы `vacancy.publish`/`test.create` берут цену из репозитория, а не из `Settings`.
- UI: раздел «Тарифы» в админке. FIRST TEST: чтение дефолтов, изменение админом, применение новой цены при списании, RBAC.

**C. Кто прошёл тест — показывать.** Частично **есть**: у откликнувшихся бейдж «Тест компании пройден» в `/org/candidates` (`passed_company_test_users`). Добавить: список прошедших **на конкретный тест** (`GET /tests/{uuid}/passers`, владелец компании) + экран на `/org/tests`. (Уточнить у заказчика: badge у откликов уже есть — нужен ли отдельный список по тесту.)

**D. Условие отклика: только прошедшие нужные тесты.**
- `Vacancy.required_test_uuids: ARRAY(UUID)` (пусто = без ограничения). Миграция autogenerate.
- Создание/редактирование вакансии: выбор обязательных тестов (тесты компании + платформенные).
- `ApplicationService.apply()`: если не пройдены все `required_test_uuids` → 409 «Сначала пройдите тест …».
- Лента/карточка смены: показать требуемые тесты; кнопка «Откликнуться» с подсказкой, если не пройдено. SDK + тесты.

**Порядок (TDD, отдельными коммитами):** сначала D (ядро продукта: гейт откликов по тестам), затем B (тарифы + встроится в редизайн `/admin`), затем C-доп (список прошедших по тесту) и добор `/admin`-shell + фикс e2e. Уточнения по C — не блок.

**Статус §11.6 (2026-07-05, выполнено):**
- **A** — `/org` и `/admin` переведены на сайдбар-шелл референса (серый фон + панель + сайдбар с шапкой и карточкой пользователя). `/admin` = лендинг «Обзор» (реальные метрики + очередь модерации/жалобы) с разделами. Данные реальные; фейковые цифры референса не переносили (real-data-only).
- **B** — модуль `app/pricing` (`service_price`), `GET/PUT /admin/pricing`, раздел «Тарифы» в админке; публикация и создание теста берут цену из БД.
- **C+D** — свёрнуто в одно (решение заказчика): у вакансии `required_test_uuids`, гейт откликов (409), выбор тестов при создании события. Опционально по событию; выбрал → жёсткий блок. Отдельный список прошедших не нужен.
- **e2e** — переведён на e-mail-OTP (Mailpit), `grant-admin` по почте, учёт нового дефолта админки. Зелёные: pytest ~145, Vitest 32, Playwright 3.

### 11.7 Карты Yandex + профили компании и исполнителя (заказчик 2026-07-05)

- **A. Yandex Maps вместо MapLibre.** Сейчас карта — `components/map-picker.tsx` (MapLibre), используется в `company-application.tsx` (заявка) и `apps/[uuid]`. Заменить на Yandex JS API v3 в пикере (заявка компании + при создании события — добавить точку) и в просмотре. **Блокер:** нужен `YANDEX_MAPS_API_KEY` (JS API) от заказчика — без ключа Yandex-карты не грузятся. Ключ → env `NEXT_PUBLIC_YANDEX_MAPS_KEY`.
- **B. Публичный профиль компании** `/company/[uuid]`: шапка (название, рейтинг), **активные события** и **отзывы**. Бэкенд почти готов: `GET /companies/{uuid}` и `GET /companies/{uuid}/reviews` уже публичны; отзывы двусторонние. Нужен **публичный список активных вакансий компании** (добавить фильтр `company_uuid` в публичный фид `GET /vacancies` или отдельный эндпоинт). Фронт: страница + ссылка с карточки смены (org+рейтинг → профиль).
- **C. Профиль исполнителя с отзывами.** `GET /users/{uuid}/reviews` уже публичен (отзывы `about_worker` создаются организацией с правом hire). На `/profile` добавить полученные отзывы + средний рейтинг + желаемые роли/пройденные тесты; по концепт-арту. (Опц.: публичный просмотр профиля соискателя организацией из карточки отклика.)

**Порядок:** B и C делаю сразу (бэкенд в основном есть). A — как только заказчик даст Yandex-ключ; до этого MapLibre остаётся рабочим.

**Статус §11.7 (2026-07-05, выполнено):**
- **A** — `map-picker` переведён на Yandex Maps JS API 2.1; ключ через `NEXT_PUBLIC_YANDEX_MAPS_KEY` (build-arg Dockerfile + compose). Проверено: карта рендерится на стенде, `ymaps` грузится без ошибок. MapLibre выведен из использования.
- **B** — публичный `/company/[uuid]` (шапка+рейтинг, активные события, отзывы); публичный фид фильтруется по `company_uuid`; ссылка с карточки смены.
- **C** — на `/profile` блок «Отзывы обо мне»; общий компонент `review-list`.
- Тесты: pytest 148, Vitest 32, tsc чисто.

### 11.8 Избранные компании + уведомления о новых сменах (заказчик 2026-07-05)

Заказчик: «ставить избранные компании, чтобы новые заявки [смены] по ним сразу отправлялись пользователю».

- **`app/favorite`** — таблица `company_favorite(user_uuid, company_uuid, uniq)`; `POST/DELETE /companies/{uuid}/favorite`, `GET /favorites/companies`. Фронт: «сердечко» на профиле компании и в карточке смены/ленте.
- **`app/notification`** — таблица `notification(user_uuid, kind, title, vacancy_uuid?, company_uuid?, read_at)`. При активации вакансии (модерация → active) создаём уведомление каждому подписчику компании. `GET /notifications` (+unread), `POST /notifications/read`. Фронт: колокольчик с бейджем в шапке → список, клик ведёт на смену.
- **Доставка «сразу»:** MVP — уведомление создаётся в момент публикации, видно по бейджу/на `/notifications`. Realtime-пуш можно доложить через существующий Redis pub/sub (как в чате) — опционально.
- FIRST TEST (pytest): подписка/отписка/список; при approve-модерации смены у подписчика появляется уведомление; unread и mark-read; не-подписчику уведомления нет.

**Статус §11.8 (2026-07-05, выполнено):** модули `app/favorite` и `app/notification`; хук в `vacancy.moderate` (approve→active) создаёт уведомления подписчикам. Фронт: «В избранное» на `/company/[uuid]`, колокольчик с бейджем в шапке, страница `/notifications`. pytest 154, Vitest 32, tsc чисто. Realtime-пуш уведомлений (через Redis pub/sub, как в чате) — опциональный доп.

### 11.9 Пополнение: подтверждение оплаты + платёжные счета с лимитами (заказчик 2026-07-05)

- **A (сделано):** админ открывает подтверждение оплаты (proof) прямо в заявке на пополнение — бэкенд уже пускал admin к `documents/{uuid}/content`, добавлена кнопка «Посмотреть чек».
- **B–C (в проработке):** реквизиты для перевода + управление платёжными счетами платформы с лимитами оборота.
  - Новый домен `payment_account`: имя, реквизиты (текст: банк/карта/получатель), `limit_kop` (лимит оборота), `received_kop` (сколько уже зачислено), активность.
  - Компания при пополнении видит реквизиты активного счёта, переводит, грузит пруф.
  - Админ при зачислении выбирает счёт-получатель; сумма прибавляется к `received_kop`; показываем «переведено X из лимита Y».
  - Если сумма превысит остаток лимита счёта — уведомление админу + выбор другого счёта.
  - **Решения заказчика (2026-07-05):** лимит — **за календарный месяц**; **приоритетный счёт назначает админ** — с него начинаем подбор при создании заявки пополняющим; если остаток месячного лимита приоритетного < суммы — берём следующий активный счёт с достаточным остатком (по убыванию запаса); если подходящего нет — заявка без счёта + **уведомление всем админам**; реквизиты — **свободный текст**.
  - Реализация: `payment_account(name, requisites, monthly_limit_kop, is_priority, active)` (приоритетный — один, назначается админом); `TopupRequest.payment_account_uuid` (nullable); оборот месяца = сумма approved-заявок по счёту за текущий месяц; создание заявки отдаёт реквизиты выбранного счёта; уведомление админам — kind `topup_no_account`. Админ-CRUD счетов + витрина «принято X из Y за месяц».

**Статус §11.9 (2026-07-05, выполнено):** A — кнопка «Посмотреть чек» в заявке на пополнение. B/C — модуль `app/payment_account` (CRUD, приоритет, месячный лимит/оборот), авто-подбор счёта при создании заявки (приоритет→запас→нет свободного=уведомление всем админам), `GET topup-requisites` для показа реквизитов до перевода. Фронт: вкладка «Счета» в админке + реквизиты в диалоге пополнения. pytest 157, Vitest 32, tsc чисто. §9.12 (review+complaint: backend, миграции, SDK, экраны, e2e) и §9.13 (admin: overview+очереди Организации/Публикации/Пополнения/Выплаты/Жалобы) закрыты. Полный e2e-цикл: регистрация организации → модерация → пополнение → публикация → отклик → найм → выплата → отзыв → жалоба. Осталось по §9: chat (п.11), затем полировка (п.15). pytest 134, Vitest 32, e2e 3 — зелёные.

### 11.10 Per-company тарифы, плата за сотрудника после смены, чат из откликов (заказчик 2026-07-06)

- **A. Тарифы per-company.** `service_price.company_uuid` (nullable) + uniq(key, company_uuid). Резолюция цены: тариф компании → глобальный override → дефолт Settings. Админ: `GET/PUT /admin/companies/{uuid}/pricing[/{key}]`; участник компании видит свои эффективные цены (`GET /companies/{uuid}/pricing`) — реальные цены на формах вместо захардкоженных «990 ₽»/«1 500 ₽».
- **B. Плата за сотрудника после мероприятия.** Новый тариф `worker_completion` (дефолт 0 — включается админом глобально или per-company). Списывается при проведении выплаты: компания → платформа, `fee × число работников`, ledger kind `completion_fee`, одна транзакция с выплатой (недостаточно средств → 409, всё откатывается).
- **C. Чат с сотрудником.** Кнопка «Чат» на карточке отклика (`/org/candidates`) — открывает/создаёт тред заявки → `/chat/[uuid]`; симметрично у соискателя в карточке заявки (проверить наличие).

**Статус §11.10 (2026-07-06, выполнено):** A — `service_price.company_uuid` (компания → глобальный → дефолт), админ-панель тарифов компании, `GET /companies/{uuid}/pricing` для команды, реальные цены на формах создания события/теста. B — тариф `worker_completion` (дефолт 0), списывается при execute_payout за каждого работника (kind `completion_fee`). C — `ChatButton` (создать/открыть тред заявки → /chat/[uuid]) у откликов организации и в карточке заявки соискателя. pytest 164, Vitest 32, tsc чисто.

### 11.11 Правка/удаление сообщений, расширенный профиль (заказчик 2026-07-06)

- **Чат:** у сообщения время отправки; правка своего сообщения → пометка «изменено» (+время), прежний текст в `chat_message_revision`; удаление — только флаг `deleted_at` (текст скрыт наружу, «Сообщение удалено»), оригинал остаётся. Версии и удалённые видны админу: `GET /chat/admin/moderated-messages` + вкладка «Чаты» в админке. Правки/удаления доезжают в открытые чаты по WS (`message_edited`/`message_deleted`). Привязка чата к событию: в шапке треда ссылка «Событие: … →» (`/shift/{vacancy_uuid}`).
...
### 11.13 Мобильная веб-версия: PWA + нижний таб-бар (заказчик 2026-07-06)
...
### 11.15 Модерация пользователей + управление пользователями в админке (заказчик 2026-07-23)

- **Контекст/пробел:** сейчас KYC живёт только на уровне документа (`document.status` pending|verified|rejected), нет модерации пользователя целиком, нет вкладки «Пользователи», нет списка всех юзеров, редактирования роли и бана. Заказчик хочет полный флоу в `/admin`.
- **Флоу модерации (заказчик):** при регистрации пользователь отправляет документ (админ может **ознакомиться** — просмотр контента) → если ок, админ **подтверждает** пользователя → если нет, либо **требует загрузить повторно** (запрос дослать документ), либо **банит**.
- **Модель `User` (миграция Alembic):** новый `moderation_status: pending | approved | resubmit | banned` (Enum `native_enum=False`, default `pending`), `moderation_reason: str|None` (причина reject/resubmit/бана), `moderated_by_uuid → user`, `moderated_at`. `is_active` остаётся, бан = `moderation_status=banned` + `is_active=False`. Гейт входа: `banned` → 403 при auth. Решение по неоднозначности: держим отдельный `moderation_status`, чтобы не смешивать с per-document `document.status`.
- **Backend (TDD, только admin, негативные тесты по RBAC):**
  - `GET /api/v1/admin/users` — список всех пользователей (фильтр `moderation_status`, поиск по email/имени, limit/offset). Отдаёт роль, статус модерации, число/статусы документов.
  - `GET /api/v1/admin/users/{uuid}` — карточка: профиль + список документов (kind/status) со ссылками на контент (переиспользуем `GET /documents/{uuid}/content`, admin уже допущен — §document/service.py:49).
  - `POST /admin/users/{uuid}/approve` · `.../resubmit` (reason) · `.../ban` (reason) · `.../unban` — смена `moderation_status`, запись `moderated_by/at`, уведомление пользователю (реюз `app/notification` + письмо через `MailingService`).
  - `PATCH /admin/users/{uuid}` — редактирование роли (`platform_role`) и базовых полей. Смена своей роли/самобан админа запрещены (защита от локаута).
  - `POST /admin/users` — ручное создание пользователя админом (**подтверждено заказчиком** «кнопка создания — пользователя»): поля email+роль, приглашение/логин через OTP-письмо, `moderation_status=approved` (создан админом).
- **Фронт `/admin` (вкладка «Пользователи»):** таблица всех юзеров (email/имя/роль/статус модерации/badge документов) + поиск/фильтр; карточка юзера — просмотр документов (открыть контент), кнопки Подтвердить / Запросить повторно / Забанить / Разбанить, селект роли (PATCH), кнопка «Создать пользователя». Пустое состояние — по real-data-only.
- **Кнопка выхода (пробел):** `logout` из `useAuth` сейчас только в `/admin` (page.tsx:1133). Добавить кнопку/пункт «Выйти» в шапку/меню обычного кабинета (`/feed`, `/org`, профиль, мобильный таб-бар/меню) — везде, где авторизованный пользователь, но выхода нет.
- **Порядок (коммиты):** тест API модерации+список → модель+миграция → admin-роутер/сервис/схемы → SDK-типы → вкладка «Пользователи» в web → кнопка «Выйти» в кабинетах → e2e (регистрация → загрузка документа → админ смотрит → approve/ban). Локаль ru, TDD, UUIDv7, RBAC на каждом эндпоинте (skill rbac-permissions), документы KYC компаниям недоступны (skill s3-documents-kyc).

**Статус §11.15 (2026-07-23, выполнено):** `User.moderation_status` (pending|approved|resubmit|banned) + reason/by/at, миграция; бан→`is_active=False` (гейт `get_current_user`). `AdminUserService` + эндпоинты `/admin/users` (список с фильтром/поиском+число документов, детали с документами, approve/resubmit/ban/unban, PATCH роли, POST создание), защита от самобана/смены своей роли. 13 pytest-сценариев. Web: вкладка «Пользователи» (карточка с документами, действия, роль, создание), стат обзора ведёт на вкладку, кнопка «Выйти» в общей шапке. Осталось (ponytail-долг): письмо/notification юзеру при approve/resubmit/ban; e2e. pytest 196, Vitest 32, tsc чисто.

### 11.16 Публичная заявка на отель без учётной записи (заказчик 2026-07-23)

- **Изменение против §10.4:** сейчас создать компанию может только авторизованный юзер (`POST /companies`, creator=main_manager — company/router.py:20). Заказчик: заявку на отель **можно отправить без учётной записи**; аккаунт заводится через админку **после модерации заявки**. Плюс к заявке можно **прикрепить документ, доказывающий должность** заявителя.
- **Модель `company_application` (новая таблица, UUIDv7 PK):** данные компании (название/город/адрес/координаты и т.п. как в текущей форме) + контакт заявителя (`applicant_email`, `applicant_name`, `applicant_position`), `status` pending|approved|rejected, `reject_reason`, `document`-ссылка (пруф должности), timestamps. Заявитель без аккаунта — храним в заявке, а не в `user`.
- **Публичный API (без auth, с антиспамом — rate-limit по IP как у OTP):**
  - `POST /api/v1/company-applications` — приём заявки (данные компании + контакт + должность). Валидация email.
  - Пруф должности: загрузка файла до создания аккаунта — `document.owner_uuid` сейчас NOT NULL → либо `company_application_uuid`-владелец (nullable owner + FK на заявку), либо отдельный `application_document`. Решение: отдельная лёгкая привязка к заявке, KYC-документы пользователя не трогаем. Публичный `POST /company-applications/{uuid}/document` (одноразовый токен из ответа на создание заявки, чтобы чужой не дозагружал).
- **Админ-API:** `GET /admin/company-applications` (фильтр по статусу), `GET /.../{uuid}` (с просмотром пруф-документа), `POST /.../{uuid}/approve` → создаёт `User`(main_manager, moderation_status=approved) + `Company`(status=verified), шлёт заявителю OTP-письмо для входа; `POST /.../{uuid}/reject` (reason) → письмо-отказ. Только admin.
- **Фронт:** публичная страница-форма «Заявка на подключение отеля» (без логина) с MapPicker + загрузкой документа-пруфа; в `/admin` — вкладка/очередь «Заявки на отель» (просмотр данных+документа, Одобрить/Отклонить). Старый путь «заявка от авторизованного» — оставить или заменить публичным (уточнить у заказчика при реализации; по умолчанию — оставить оба, публичный основной).
- **Порядок (коммиты):** тесты публичного приёма+модерации → модель+миграция `company_application` → публичный роутер+пруф-документ → admin-роутер (approve создаёт user+company) → SDK → публичная форма → вкладка в админке → e2e (аноним шлёт заявку с документом → админ одобряет → заявитель логинится по OTP → попадает в `/org`).

**Статус §11.16 (2026-07-23, выполнено):** модуль `app/company_application` (таблица+миграция, repo, `PublicApplicationService`+`AdminApplicationService`, публичный и admin роутеры). Публично: `POST /company-applications` (JSON, переиспользует валидаторы `CompanyCreateIn`) + `POST /{uuid}/document` по одноразовому `upload_token`; пруф в Storage отдельным ключом (не Document — тому нужен владелец-юзер). Admin: список/пруф/approve/reject; approve создаёт `Company(verified)`+`User(main_manager)` и открывает кабинет, вход по OTP на контактную почту. 8 pytest-сценариев. Web: публичная `/apply` (форма+MapPicker+загрузка пруфа), ссылка «Подключить отель» в шапке, админ-вкладка «Заявки на отель». Осталось (ponytail-долг): письмо-приглашение/отказ заявителю; rate-limit по IP на публичном приёме; e2e. pytest 196, Vitest 32, tsc чисто.

**Отложенный долг (§11.15+§11.16):** письма/уведомления при модерации; IP-антиспам на публичном приёме заявок; e2e-сценарии обоих модулей. Помечено `ponytail:` в `company_application/service.py` и `admin/service.py`.
