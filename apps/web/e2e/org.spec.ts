...
import { grantAdmin, loginByEmail } from "./helpers";
...
test("полный цикл: организация → модерация → публикация → отклик → найм", async ({ browser }) => {
  ...
  // точка на карте: клик по центру Yandex-карты ставит маркер
  await manager.locator('[data-testid="map-picker"]').click();
  ...
  // — менеджер нанимает (итог за смену уходит в резерв под выплату)
